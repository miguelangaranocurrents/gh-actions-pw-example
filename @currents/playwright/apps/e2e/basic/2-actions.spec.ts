 ...
 */
test("basic interaction", async ({ page }, testInfo) => {
  ...
  await page.screenshot({ path: "./screenshots/s-3.png" });
  await page.screenshot({ path: "./screenshots/s-4.png" });
  // await expect(todoList).toHaveText("blah blah");
});
